...
import logoImg from '../resources/img/L3.png';
import logoLightImg from '../resources/img/L3.png';
...
const Logo: React.FC<LogoProps> = ({ img = logoImg, imgLight = logoLightImg, className = 'flex-shrink-0' }) => {
    return (
        <Link to="/" className={`ttnc-logo inline-block text-slate-600 ${className}`}>
            ...
            {img ? <img className={`block max-h-8 sm:max-h-10 ${imgLight ? 'dark:hidden' : ''}`} src={img} alt="Logo" /> : 'Logo Here'}
